...
1. 先运行 `node SKILL_ROOT/runtime/tools/useless-linkedin.mjs tracker --history`，再运行 `node SKILL_ROOT/runtime/tools/useless-linkedin.mjs scan`；搜索配置只改 `.useless-linkedin/portals.yml`。
...
4. 新线索进入 `process-job.md`；扫描本身不代表投递，也不在 Excel 中虚构申请行。

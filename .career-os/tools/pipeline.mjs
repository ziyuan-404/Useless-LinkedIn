...
import {validateAssessment} from './lib/schema.mjs';
...
try{
 ...
 const result=JSON.parse(await fs.readFile(path.resolve(root,a.assessment),'utf8'));await validateAssessment(result);if(result.contextHash!==contextHash)throw Error('Assessment is stale: JD or facts changed');
 ...
 if(result.decision?.route==='bulk'){
   const resume=spawnSync(process.execPath,[path.join(toolsRoot,'select-resume.mjs'),'--family',result.decision.resumeFamily||''],{encoding:'utf8'});
   ...
   else{const output=resume.stdout.trim();await set({state:'materials-pending-review',ko,priority:result.priority,company:result.company,title:result.role,output,applicationMode:'bulk'});console.log(JSON.stringify({id:job.id,ko,route:'bulk',output,review:'resume suitability review required'}));}
 }else{

...
状态值的唯一机器定义是 `.career-os/tools/lib/state-machine.mjs`。运行 `node .career-os/tools/career.mjs state --list-states` 可查看当前合法值；`schemas/state.schema.json` 与之同步。历史记录的旧值保留，不因文档改名而无证据升级。
...
共享命令 `node .career-os/tools/career.mjs state --id ID --to STATE [--evidence TEXT] [--reason TEXT]` 负责人工操作后的状态更新；进入 `submitting` 或 `submitted` 会检查该岗位的提交授权。`submitted` 的 evidence 应指向可核验的成功页、确认邮件或平台状态，不得填“已点击”。

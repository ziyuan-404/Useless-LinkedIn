...
try{
 ...
 const files=['00-个人资料/profile/basics.md','00-个人资料/profile/preferences.md','00-个人资料/profile/links.md','00-个人资料/profile/claim-map.md',...(await fs.readdir(path.join(root,'00-个人资料/profile/experiences'))).filter(x=>x.endsWith('.md')).map(x=>`00-个人资料/profile/experiences/${x}`),'00-个人资料/operations/application-rules.md','00-个人资料/operations/resume-strategy.md'];
 ...
 if(result.decision?.route==='bulk'){
   ...
   else{const output=resume.stdout.trim();await set({state:'materials-pending-review',ko,priority:result.priority,company:result.company,title:result.role,output,applicationMode:'bulk',contextHash});console.log(JSON.stringify({id:job.id,ko,route:'bulk',output,review:'resume suitability review required'}));}
 }else{
 if(!result.payload)throw Error('PASS requires material payload');await checkSources(result.payload.cv,[path.join(root,'00-个人资料/profile')]);await checkSources(result.payload.letter,[path.join(root,'00-个人资料/profile'),path.join(dir,'jd.txt')]);
 ...
 const output=path.join(root,'00-个人资料/CV',`${new Date().toISOString().slice(0,10)}-${job.id}-${hash(contextHash+JSON.stringify(result.payload)).slice(0,8)}`);
 ...
 await set({state:'materials-pending-review',ko,priority:result.priority,company:result.company,title:result.role,output,submitted:false,contextHash});console.log(JSON.stringify({id:job.id,ko,output,review:'semantic and visual review required',questions:path.join(dir,'questions.json')}));
 }
 }
 }
 }

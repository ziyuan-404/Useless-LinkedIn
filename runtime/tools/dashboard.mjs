...
import {root,args,artifactTool} from './runtime.mjs';
const a=args();
...
try{
 ...
 console.log(JSON.stringify(patch));if(!a.apply){console.log('Preview only; workbook unchanged');process.exitCode=0;}
  ...
  const dir=path.join(root,'.useless-linkedin/dashboard-backups');await fs.mkdir(dir,{recursive:true});const token=Date.now();

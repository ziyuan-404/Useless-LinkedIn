...
if(a.grant){
  ...
  if(selected.includes('submit')&&typeof a['approval-snapshot-id']!=='string')throw Error('Submit grant requires --approval-snapshot-id from the reviewed materials');
  const grant={id:randomUUID(),source:a.source,recordedAt:new Date().toISOString(),scope:{kind:'job',jobId:a['job-id']},actions:Object.fromEntries(selected.map(x=>[x,true])),...(a['approval-snapshot-id']?{approvalSnapshotId:a['approval-snapshot-id']}:{}) ,...(a['expires-at']?{expiresAt:a['expires-at']}:{})};
...
}).filter(g=>g.actions[a.check]===true&& (a.check!=='submit'||typeof a['approval-snapshot-id']==='string'&&g.approvalSnapshotId===a['approval-snapshot-id']));

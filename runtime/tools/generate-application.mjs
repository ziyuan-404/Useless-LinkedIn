...
const basics=await fs.readFile(path.join(root,'.useless-linkedin/profile/basics.md'),'utf8');const candidateName=basics.match(/^-\s*姓名[:：]\s*(.+)$/m)?.[1]?.trim();if(!candidateName||/待填写|to complete/i.test(candidateName))throw Error('Configure a confirmed candidate name in the local profile');
...
for(const [kind,allowed] of [['cv',['.subtitle','.profil-text','.skill-bullets','.profil-header-target','.availability','.item-title','.item-date','.item-sub','.item-loc','.item-bullets','.course-list']],['letter',['.subject','.letter','.personal-info']]]){
 ...
 for(const item of payload[kind]){
  ...
  for(const source of item.sources){const p=path.resolve(root,source.path);if(!p.startsWith(path.join(root,'.useless-linkedin')+path.sep))throw Error('Source outside .useless-linkedin');const text=await fs.readFile(p,'utf8');if(typeof source.quote!=='string'||source.quote.length<8||!text.includes(source.quote))throw Error(`Source quote missing: ${source.path}`);}
 }
}
...
try{
 await fs.mkdir(out);await fs.mkdir(path.join(out,'work'));await fs.copyFile(path.join(root,'.useless-linkedin/template/portrait.png'),path.join(out,'portrait.png'));
 for(const [kind,template,name] of [['cv','resume','CV'],['letter','motivation-letter','Lettre']]){
  ...
  const html=path.join(out,`${template}.html`);await fs.copyFile(path.join(root,'.useless-linkedin/template',`${template}.html`),html);
